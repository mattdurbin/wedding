  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/upload" && request.method === "POST") {
      try {
        ...
        return json({ error: err.message || "Unexpected error while processing your submission." }, 500, request);
      }
    }
...
async function handleUpload(request, env) {
  ...
  if (uploads.length > MAX_FILES) {
    return json({ error: `Please upload no more than ${MAX_FILES} files at once.` }, 400, request);
  }
  ...
  for (const file of uploads) {
    if (file.size > MAX_FILE_SIZE) {
      return json({ error: `File too large: ${file.name}` }, 400, request);
    }
...
function redactSensitiveText(text, contact = "") {
  ...
  // Redact UK/international-ish phone number patterns
  redacted = redacted.replace(/(?<!\w)(?:\+?\d[\d\s().-]{7,}\d)(?!\w)/g, (match) => {
    const digits = match.replace(/\D/g, "");
    return digits.length >= 9 ? "[redacted phone]" : match;
  });

  // Redact exact submitted contact value if it appears in the message
